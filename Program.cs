...
namespace JasonDemo
{
    class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("Unable to setup working folder");
            ...
            var killExcels = ConfigurationManager.AppSettings["PreventKillExcelsAtStart"];
            if (!string.IsNullOrEmpty(killExcels) && killExcels.ToLower().Trim() != "true")
                CloseAllExcels();
            ...
            var portfolio = ConfigurationManager.AppSettings["portfolioid"];
            ...
            var XL2 = new Application();
            if (File.Exists(pathCosts))
            {
                Log($"Opening Excel Doc " + pathCosts);
                wbCosts = XL2.Workbooks.Open(pathCosts);
            }
            else
                Log("Could not find file " + pathCosts);
        ...
        private static void LoadPeriods(int periods)
        {
        ...
        private static void CopyAttributeValues(Story portfolio, Item portolioItem, Story projectStory, string text)
        {
            // find the item
        ...
        private static bool IsPerdiodFirstDay()
        {
            try
            {
        ...
        private static void LoadPeriodDates(Story story)
        {
            try
            {
        ...
        private static void CopyAllItemDataToNextPeriod(Story story)
        {
            try
            {
        ...
        private static void CopySpecificItemDataToNextPeriod(Story story)
        {
            try
            {
        ...
        private static void RunPendingStoryChanges(Story story)
        {
            try
            {
        ...
        private static void RunStoryCalcsForNazir(Story story)
        {
        ...
        private static void RunStoryCalcs(Story story)
        {
        ...
        private static void RunEFCCals(Story story)
        {
            try
            {
        ...
        private static SC.API.ComInterop.Models.Attribute GetAttribute(Story story, string id)
        {
        ...
        private static void ImportRisks(Story story, string projID)
        {
            try
            {
...
//                string filelocation = string.Format(ConfigurationManager.AppSettings["RiskXLSLocation"], _period, projID);
        ...
        private static void ImportBaseslineCosts(Application XL, Workbook wb, Story story, string projID)
        {
            if (wb == null)
            {
                return;
        ...
        private static void LoadMilestones(Application XL, Workbook wb)
        {
        ...
        private static void ImportMilestones(Story story, string projID)
        {
        ...
        private static void GetStoryPanels()
        {
            var userid = ConfigurationManager.AppSettings["userid"];
            var passwd = ConfigurationManager.AppSettings["passwd"];
        ...
        private static void SetStoryPanels()
        {
            var userid = ConfigurationManager.AppSettings["userid"];
            var passwd = ConfigurationManager.AppSettings["passwd"];
        ...
        private static void SetStoryPanels(Story story)
        {
            foreach (var i in story.Items)
            {
                SetPanelData(i);
        ...
        private static void SetPanelData(Item item)
        {
            var data = "";
            var title = "";
            var data2 = "";
